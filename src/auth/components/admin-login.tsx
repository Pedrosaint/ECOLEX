...
export const AdminLogin = () => {
  ...
  // Updated AdminLogin component - using backend functions only
  const onSubmit = async (data: SuperAdminLogin) => {
    console.log("Form submission started with data:", data);
    setIsLoading(true);
    ...
    try {
      ...
      console.log("Backend setup progress check:", {
        hasIncompleteSetup,
        currentStep,
        shouldRedirect: hasIncompleteSetup,
      });

      if (hasIncompleteSetup) {
        // Redirect to the appropriate setup step based on backend step data
        switch (currentStep) {
          case 1:
