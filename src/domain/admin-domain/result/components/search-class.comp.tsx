  ...
  return (
    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4 }}>
        ...
        <div>
          ...
          <div className="relative">
            <select value={classGroupId} onChange={(e) => setClassGroupId(e.target.value)} disabled={!classId || groupsLoading} className={`${selectBase} ${classGroupId ? "text-gray-900" : "text-gray-400"} ${!classId ? disabledCls : ""}`}>
              <option value="">{!classId ? "Select a class first" : groupsLoading ? "Loading..." : filteredGroups.length === 0 ? "No groups for this class" : "Select Group"}</option>
      ...
      <button
        ...
        className={`w-full py-3 flex items-center justify-center gap-2 font-semibold text-white transition-colors ${canSearch && !isSearching ? "bg-[#8000BD] hover:bg-[#640094] cursor-pointer" : "bg-[#D9D9D9] cursor-not-allowed"}`}
      >
