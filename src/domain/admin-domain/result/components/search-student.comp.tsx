  ...
  return (
    <motion.div className="">
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-6 mb-6">
        ...
        <div>
          <label className="block text-sm font-semibold font-inter text-gray-700 mb-2">Select Term <span className="text-red-500">*</span></label>
          <div className="relative">
            <select value={termId} onChange={(e) => setTermId(e.target.value)} disabled={!sessionId} className={`${selectBase} ${termId ? "text-gray-900" : "text-gray-400"} ${!sessionId ? "bg-gray-50 cursor-not-allowed" : ""}`}>
              <option value="">{!sessionId ? "Select session first" : terms.length === 0 ? "No terms for session" : "Select Term"}</option>
        ...
        <div>
          <label className="block text-sm font-semibold font-inter text-gray-700 mb-2">Select Student <span className="text-red-500">*</span></label>
          <div className="relative">
            <select value={studentId} onChange={(e) => setStudentId(e.target.value)} disabled={!classId || studentsLoading} className={`${selectBase} ${studentId ? "text-gray-900" : "text-gray-400"} ${!classId ? "bg-gray-50 cursor-not-allowed" : ""}`}>
              <option value="">{!classId ? "Select a class first" : studentsLoading ? "Loading students..." : (studentsData?.students ?? []).length === 0 ? "No students in this class" : "Select Student"}</option>
